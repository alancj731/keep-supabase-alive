...
class ApiTokenFilterTest {
    ...
    @Test
    void acceptsEitherTheApiTokenOrCronSecret() throws Exception {
        ApiTokenFilter twoTokenFilter = new ApiTokenFilter("manual-token", "cron-secret");
        for (String token : List.of("manual-token", "cron-secret")) {
            MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/keepalive/run");
            request.addHeader(HttpHeaders.AUTHORIZATION, "Bearer " + token);
            MockHttpServletResponse response = new MockHttpServletResponse();

            twoTokenFilter.doFilter(request, response, new MockFilterChain());
